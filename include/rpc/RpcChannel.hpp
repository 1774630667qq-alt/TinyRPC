 ...
 * @Date: 2026-05-02 09:39:44
 * @LastEditors: Zhang YuHua 1774630667@qq.com
 * @LastEditTime: 2026-05-04 08:18:32
 * @FilePath: /TinyRPC/include/rpc/RpcChannel.hpp
 ...
 *
 * 核心设计：
 * - CallMethod 不再阻塞等待回包，而是将 {response, done} 存入 outstandings_
 * map， 通过 TcpClient 发送请求后立即返回
...
public:
  ...
  /**
   * @brief 异步发起 RPC 调用
   * @signature void CallMethod(const MethodDescriptor*, RpcController*, const
   * Message*, Message*, Closure*)
   * @param method     Protobuf 方法描述符（自动获取 service_name 和
   * method_name）
   * @param controller RPC
   * 控制器指针（可选，用于在回包时记录调用状态和错误信息）
...
private:
  ...
  void onRpcResponse(const std::shared_ptr<TcpConnection> &conn,
                     const tiny_rpc::RpcMeta &meta, std::string raw_body);
  ...
  std::unordered_map<uint32_t, OutstandingCall>
      outstandings_; ///< 等待回包的请求表
};
